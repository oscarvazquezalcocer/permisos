...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $archivoTmp = $_FILES['adjunto']['tmp_name'];
    $rutaDestino = '../Adjuntos_tmp/' . $_FILES['adjunto']['name'];
    move_uploaded_file($archivoTmp, $rutaDestino);

    $rutaAdjunto = '../Adjuntos_tmp/' . $_FILES['adjunto']['name'];
    ...
    if (isset($_SESSION['username'])) {
        ...
        if ($result->num_rows == 1) {
            ...
            $sql = "INSERT INTO solicitud (User_ID, Tipo_solicitud_ID, de_fecha, a_fecha, reposicion, fecha_r1, fecha_r2, motivo, otro, adjunto, Estado_ID)
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
            ...
            $stmt->bind_param("iissssssssi", $userID, $tipoSolicitudID, $fecha1, $fecha2, $reposicion, $fechaRep1, $fechaRep2, $motivo, $otroM, $rutaAdjunto, $estadoID);
